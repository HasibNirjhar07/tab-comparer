...
const Index = () => {
  ...
  const handleAiComparison = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ detail: "AI comparison failed" }));
        throw new Error(errorData.detail || "AI comparison failed");
      }
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to perform AI comparison";
      toast({
        title: "Error",
